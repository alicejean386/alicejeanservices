...
frmSearch.search(a))},show:function(){var a=toolBar.getTopHeight(!0);this.menu.animate({top:a},300);this.visible=!0},hide:function(){this.menu.animate({top:-60},300);this.visible=!1},showOrHide:function(){!0===this.visible?this.hide():this.show()},changeLanguage:function(){this.searchBarInput.attr("placeholder",FormInfo.getFormTitle("search"))}});var PcSearchItem=Class({create:function(a,b){b.fontColor=this.mainColor;this._super(a,b)}}).extend(SearchItem);var PcSearchForm=Class({nailUpIcon:null,nailUpStatus:!1,create:function(a){this.nailUpStatus=bookConfig.isSearchFormNailUpEnabled;this.elements=new SearchElements(this);this._super(a)},initNailUpIcon:function(){this.nailUpIcon=$("<img alt='nail up' role='button' width='auto' height='auto' class='button nailUp "+(this.nailUpStatus?"nailUpActive":"")+"' src='"+(form_icons.nailUp?"data:image/png;base64,"+form_icons.nailUp:uiBaseURL+"nailUp.png")+"'/>");this.stage.append(this.nailUpIcon);this.nailUpIcon.bind(_event._click,
function(){this.nailUpIcon.toggleClass("nailUpActive");this.nailUpStatus=!this.nailUpStatus;var a=$(".search_form").outerWidth(!0),b=parseInt(_get(global,"staticAd.adPosition",1));this.nailUpStatus?(bookConfig.leftMargin+=a+10,window.staticAd&&window.staticAd.haveAd&&0<window.staticAd.data.length&&[1,2].includes(b)&&$("#slide-show").css({width:"calc(100% - 10px - "+a+"px)",right:"0px"})):(bookConfig.leftMargin-=a+10,window.staticAd&&window.staticAd.haveAd&&0<window.staticAd.data.length&&[1,2].includes(b)&&
$("#slide-show").css({width:"100%",right:"0px"}));onStageResize()}.bind(this))},onCreate:function(){this.initNailUpIcon();this.stage.css({"background-color":this.mainColorObj.rgba(.95*this.formOpacity),color:this.fontColor});this.searchResult.css({"border-bottom":"1px solid "+this.mainColorObj.rgba(.1)});this.progressBar.css({"background-color":this.fontColor})},initHtml:function(){this.stage.append(this.title);this.favicon&&(bookConfig.RightToLeft?this.title.append(this.favicon):this.title.prepend(this.favicon));
